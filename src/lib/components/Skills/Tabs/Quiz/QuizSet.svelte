...
<li class="set">
    <div class="head"  on:click={() => expanded = !expanded}>
        <span class="content" contenteditable="" on:keyup={(e) => saveTitle(e.target.innerHTML)}>{set.title}</span>
    </div>
    <div class="toggle-ai-maker btn btn-warning" on:click={() => aiMaker = !aiMaker}>
        <i class="fa fa-bolt"></i>
    </div>
    {#if expanded}
    <div class="remove" on:click={() => removeSet(set)}><i class="fa fa-trash"></i></div>
        {#if aiMaker}
            <div class="ai-maker" >
            ...
            <div class="adder">
                <div class="add-quiz btn btn-outline-warning" on:click={() => [...quiz_sets, {
                    title: "New One"
                }]}>+</div>
                <div class="btn btn-outline-warning generate-quiz" on:click={() => generateQuiz("general")}>
                    <i class="fa fa-bolt" /> General
                </div>
                <div class="btn btn-outline-warning generate-quiz" on:click={() => generateQuiz('jeopardy')}>
                    <i class="fa fa-bolt" /> Jeopardy
                </div>
                <div class="btn btn-outline-warning generate-quiz" on:click={() => generateQuiz("general_mc")}>
                    <i class="fa fa-bolt" /> <i class="fa fa-list"></i> General
                </div>
                <div class="btn btn-outline-warning generate-quiz" on:click={() => generateQuiz('techniques_mc')}>
                    <i class="fa fa-bolt" /> <i class="fa fa-list"></i> Techniques
                </div>
                <div class="btn btn-outline-warning generate-quiz" on:click={() => generateQuiz('abstractions_mc')}>
                    <i class="fa fa-bolt" /> <i class="fa fa-list"></i> Abstractions
                </div>
                <div class="btn btn-outline-warning generate-quiz" on:click={() => generateQuiz('steps')}>
                    <i class="fa fa-bolt" /> <i class="fa fa-list"></i> Steps
                </div>
